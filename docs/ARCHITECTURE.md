...
| -------------- | ------ | -------------- | ---------- |
| Intent Classifier | `app/ai/intent_classifier.py` | 52 rules over 6 intents, optional model refinement | rules (+ model to raise confidence) |
...
| Risk Classifier | `app/ai/risk_classifier.py` | 42 rules mapping signals to a risk level, plus an incident-shape backstop | rules (+ model may **raise**, never lower) |
